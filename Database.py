...
# Base class
class Item:
    def __init__(self, label, color, secondImage, description = "No description provided", location = ""):
...
# Main function
def inputData(label, color, description, location, secondImage):
    label = label.lower()
    if label not in item_classes:
        raise ValueError(f"Unknown item label: {label}")

    item_class = item_classes[label]
    item_instance = item_class(label, color, description, location, secondImage)
